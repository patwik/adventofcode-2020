...
input pcond : $50. pstring : $50.;
pcharmin=scan(pcond,1,'- ');
pcharmax=scan(pcond,2,'- ');
pchar=scan(pcond,3,'- ');
pantal=countc(pstring,pchar,'t');
if pantal >=pcharmin and pantal <=pcharmax then 
 numvalid=sum(numvalid,1); 
   put pcond= pstring= pcharmin= pcharmax= pchar= pantal= numvalid=;
datalines;
*/
data kalle;
  infile datalines dlm=':' truncover;
  input pcond : $50. pstring : $50.;
    ppos1=input(scan(pcond,1,'- '),best.);
    ppos2=input(scan(pcond,2,'- '),best.);
    pchar=scan(pcond,3,'- ');
    ppchar1=substr(pstring,ppos1,1);
    ppchar2=substr(pstring,ppos2,1);
  if (ppchar1=pchar or ppchar2=pchar) and ppchar1 ne ppchar2 then numvalid+1;
datalines;
